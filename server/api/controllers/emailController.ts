...
import { BookmarkDeliveryJob } from "../jobs";
    ...
    const artworkHashMap = bookmarkSet.reduce<CollectedArtworks>(
      (acc, bookmark) => {
        ...
        // needed by the bookmark. It is a problem if this is not the case, and we want to know
          ...
          console.warn(
            `[EmailController.getStoryEmail] Could not pull Artwork with Image ID ${bookmark.image_id} from the 'artworksForBookmarks' list for Session ID ${sessionId}`
          );
        }
        return acc;
      },
