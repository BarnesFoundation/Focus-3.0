...
class SnapResults extends Component {
  ...
  constructResultAndInRoomSlider = (search_result) => {
    if (search_result["success"]) {
      ...
      if (search_result["data"]["records"].length > 0) {
        ...
        let cropParams =
          "?q=0&auto=compress&crop=faces,entropy&fit=crop&w=" + w;
        ...
        result["curatorialApproval"] =
          art_obj.curatorialApproval === "false" ? false : true;
      }
  ...
  onSelectLanguage = async (lang) => {
    console.log(
      "Selected lang changed in SnapResults component : " + JSON.stringify(lang)
    );
    ...
    let artworkInfo = await this.sr.getArtworkInformation(
      this.state.searchResults[0].id
    );
    this.setState({ result: artworkInfo });
    this.constructResultAndInRoomSlider(artworkInfo);
  };
  ...
  updateBackgroundStyle = () => {
    if (this.state.bgLoaded && this.resultsContainer) {
      ...
      let artworkBgBottom =
        this.artworkBackgroundContainer.getBoundingClientRect().bottom;
  ...
  handleScroll = () => {
    ...
    let resultsContainerBottom = Math.ceil(
      h - this.resultsContainer.getBoundingClientRect().bottom
    );
    ...
    let shortDescElemHeight =
      shortDescBoundingRect.bottom - shortDescBoundingRect.top;
    ...
    if (visibleShortDescHeight < 0) {
      ...
      visibleShortDescHeight > 60 &&
      visibleShortDescHeight <= shortDescElemHeight
    ) {
      this.setState({
  ...
  getFacebookShareUrl = () => {
    let urlToShare =
      "https://collection.barnesfoundation.org/objects/" +
      this.state.searchResults[0].id;
    return (
    ...
    switch (socialMediaType) {
      case constants.SOCIAL_MEDIA_TWITTER: {
        ...
        if (this.state.searchResults[0].artist) {
          ...
          hashtag +=
            "," +
            this.state.searchResults[0].artist
              .split(" ")
              .join("")
              .split("-")
              .join("");
  ...
  _onClickShare = () => {
    if (navigator.share) {
      ...
      if (this.state.searchResults[0].artist) {
        title_author += " by " + this.state.searchResults[0].artist;
        hashtag +=
          " #" + this.state.searchResults[0].artist.split(" ").join("");
      }
    ...
    return (
      <div>
              ...
              >
                <div className="card-top-container">
                  <div
                    ...
                    ref={(el) => (this.artworkBackgroundContainer = el)}
                    style={bgImageStyle}
                  >
                    <img
                ...
                >
                      ...
                      <tbody>
                        <tr>
                          <td className="text-left item-label">
                            {this.props.getTranslation("Result_page", "text_3")}
                            :
                          </td>
                        ...
                        <tr>
                          <td className="text-left item-label">
                            {this.props.getTranslation("Result_page", "text_4")}
                            :
                          </td>
                          <td className="text-left item-info">
                            {artwork.title}
                        ...
                        <tr>
                          <td className="text-left item-label">
                            {this.props.getTranslation("Result_page", "text_5")}
                            :
                          </td>
                          <td className="text-left item-info">
                            {artwork.displayDate}
                        ...
                        <tr>
                          <td className="text-left item-label">
                            {this.props.getTranslation("Result_page", "text_6")}
                            :
                          </td>
                          <td className="text-left item-info">
                            {artwork.medium}
                        ...
                        <tr>
                          <td className="text-left item-label">
                            {this.props.getTranslation("Result_page", "text_7")}
                            :
                          </td>
                          <td className="text-left item-info">
                            {artwork.dimensions}
                  ...
                  <div className="share-wrapper">
                    <div className="language-dropdown-wrapper">
                      {this.state.isLanguageDropdownOpen && (
                        <div className="language-dropdown-overlay" />
                      )}
                      ...
                      <PopoverBody>
                        <div className="share">
                          <a
                            ...
                            onClick={this.nativeAppShareWithWebFallback}
                          >
                            <i
                ...
                {this.state.alsoInRoomResults.length > 0 && (
                  <Child
                    ...
                    style={this.state.slideOverStyle}
                  >
                    <InRoomSlider
                      alsoInRoomResults={this.state.alsoInRoomResults}
                      blurValue={this.state.blurValue}
                      onSelectInRoomArt={this.onSelectInRoomArt}
        ...
        {this.state.showAboutScreen && (
          <div>
            <About
              onCloseAbout={this._onCloseAbout}
              getTranslation={this.props.getTranslation}
            />
          </div>
        )}
      ...
      <div>
        {!this.state.bgLoaded && (
          <div style={{ visibility: `hidden` }}>
            <img
              className="card-img-top"
              src={this.state.searchResults[0].bg_url}
              alt="match_image_background"
