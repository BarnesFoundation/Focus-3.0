...
class InRoomSlider extends Component {
  constructor(props) {
    ...
    this.sliderCropParams =
      "?q=0&auto=compress&crop=faces,entropy&fit=crop&h=230&w=230";
    this.sliderBackgroundCropParams =
      "?q=0&auto=compress&crop=faces,entropy&fit=crop&h=540&w=" + screen.width;
  ...
  componentDidMount() {
    ...
    this.props.alsoInRoomResults.map((record, index) => {
      ...
      aitrMatchImage.src =
        this.props.alsoInRoomResults[index].art_url + this.matchImageParam;
      aitrBGImage.src =
        this.props.alsoInRoomResults[index].art_url +
        this.sliderBackgroundCropParams;
    });
  }
    ...
    return (
      <div>
        <div className="slider-background" style={sliderBackground}>
          <CrossfadeImage
            src={
              this.props.alsoInRoomResults[this.state.activeSlideIndex]
                .art_url + this.sliderBackgroundCropParams
            }
            duration={1000}
        ...
        <div className="slider-header h2">
          {this.props.getTranslation("Result_page", "text_2")}
        </div>
            ...
            {this.props.alsoInRoomResults.map((record, index) => (
              ...
              >
                ...
                <ProgressiveImage
                  ...
                  placeholder={record.art_url + this.sliderCropParams}
                >
                  {(src) => (
                    <img
                      src={src}
                      id={"aitr-card-" + record.id}
                      alt="aitr_image"
                    />
                  )}
                </ProgressiveImage>
