...
type CameraProps = {
  snaps: any;
  processImageCapture: (args: any) => any;
  beginScanning: any;
...
export const Camera: React.FC<CameraProps> = ({
  ...
  const [snapAttempts, setSnapAttempts] = useState(snaps);
  const [cameraPermission, setCameraPermission] = useState(false);
  const [captureCounter, setCaptureCounter] = useState(0);
  const [cameraCapabilities, setCameraCapabilities] = useState();
  const [previewCanvas, setPreviewCanvas] = useState<any>();
  const [error, setError] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string>();
  const [drawPreview, setDrawPreview] = useState<NodeJS.Timeout>();
  const [tempCanvas, setTempCanvas] = useState<HTMLCanvasElement>();
  const [canvas, setCanvas] = useState<HTMLCanvasElement>();
  const [tempCtx, setTempCtx] = useState<CanvasRenderingContext2D>();
  const [ctx, setCtx] = useState<CanvasRenderingContext2D>();
  const [initVideo, setInitVideo] = useState<NodeJS.Timeout>();
  const [scan, setScan] = useState<NodeJS.Timer>();
  ...
  const video: React.Ref<HTMLVideoElement> = useRef(null);
  const vpreview: React.Ref<HTMLCanvasElement> = useRef(null);
  const { getTranslation } = useContext(TranslationContext);

  const videoStyle = { filter: `blur(25px)`, transform: `scale(1.2)` };

  const resetSnapCounter = useCallback(() => {
    let lastSnapTimestamp = parseInt(getLocalStorage(SNAP_LAST_TIMESTAMP));

    if (lastSnapTimestamp) {
      let ttl = lastSnapTimestamp + SNAP_COUNT_RESET_INTERVAL - Date.now();
      if (ttl <= 0 && snapAttempts > 0) {
        removeLocalStorage(SNAP_ATTEMPTS);
        setSnapAttempts(0);
      }
    }
  }, [getLocalStorage, removeLocalStorage, snapAttempts]);

  /** Captures a single scan and returns blob of the scan */
  const captureSingleScan = useCallback(async () => {
    ...
    if (captureCounter < 10) {
      ...
      processImageCapture(imageBlob);
    }
  }, [captureCounter, previewCanvas, processImageCapture]);

  /** Return a temporary canvas and context for drawing the video */
  const getTempPreviewCanvas = useCallback(() => {
    const rect = video.current.getBoundingClientRect();
    if (!tempCanvas) {
      let tCanvas = document.createElement("canvas");
      let tCtx = tCanvas.getContext("2d", { alpha: false });
      tCanvas.width = rect.width;
      tCanvas.height = rect.height;

      setTempCanvas(tCanvas);
      setTempCtx(tCtx);

      return { tCanvas, tCtx };
    } else {
      return { tCanvas: tempCanvas, tCtx: tempCtx };
    }
  }, [tempCanvas, tempCtx]);

  const stopPreview = useCallback(() => {
    if (drawPreview) {
      clearTimeout(drawPreview);
    }
  }, [drawPreview]);
  ...
  const drawVideoPreview = useCallback(() => {
    try {
      if (!vpreview.current /*  || this.props.sessionYieldedMatch === null */) {
        return null;
      }

      let { tCanvas, tCtx } = getTempPreviewCanvas();

      let preview = vpreview.current;
      let previewContext = preview.getContext("2d", { alpha: false });
      preview.width = cropRectangle.width;
      preview.height = cropRectangle.height;

      tCtx.clearRect(0, 0, tCanvas.width, tCanvas.height);
      tCtx.drawImage(video.current, 0, 0, tCanvas.width, tCanvas.height);
      ...
      setPreviewCanvas(previewContext.canvas);
      ...
      // TODO: uncomment this when we fix Google Analytics
      // ga("send", {
      //   hitType: "event",
      //   eventCategory: GA_EVENT_CATEGORY.CAMERA,
      //   eventAction: GA_EVENT_ACTION.SCAN,
      //   eventLabel: GA_EVENT_LABEL.SCANNER_MOUNT_FAILURE,
      // });
    }
  }, [cropRectangle, getTempPreviewCanvas, stopPreview]);
  ...
  const playVideo = useCallback(async () => {
    if (video.current) {
      try {
        ...
        setCameraCapabilities(track.getCapabilities && track.getCapabilities());
  ...
  }, [drawVideoPreview, video, videoStream]);

  /** Gets a blank canvas of same size as the video */
  const getCanvas = () => {
    if (video.current && !video.current.videoHeight) return null;

    if (!ctx) {
      const tCanvas = document.createElement("canvas");
      const aspectRatio = video.current.videoWidth / video.current.videoHeight;

      tCanvas.width = video.current.clientWidth;
      tCanvas.height = video.current.clientWidth / aspectRatio;

      setCanvas(tCanvas);
      setCtx(tCanvas.getContext("2d", { alpha: false }));
  ...
  const getVideoCanvas = () => {
    ...
    if (!video.current || !canvas) return null;

    const context = canvas.getContext("2d", { alpha: false });

    if (isIOS || !cameraCapabilities) {
      // Draw rectangle
      let rectangle = video.current.getBoundingClientRect();
      setTempCanvas(document.createElement("canvas"));
  ...
  const setupForCapturing = useCallback(async () => {
    ...
    setScan(
      await new Promise((resolve) => {
        setTimeout(() => {
          const scan = setInterval(captureSingleScan, 1000 / 3);
          resolve(scan);
        }, 500);
      })
    );
  }, [videoStream, stopVideo, captureSingleScan, playVideo]);
  ...
  const pinchZoomModifier = (action) => {
    const cameraContainer = document.querySelector(".camera");

    if (action === DISABLE_ZOOM) {
      cameraContainer.addEventListener("touchmove", disablePinchZoom, false);
    }

    if (action === ENABLE_ZOOM) {
      cameraContainer.removeEventListener("touchmove", disablePinchZoom);
    }
  };

  useEffect(() => {
    // Disable zoom
    pinchZoomModifier(DISABLE_ZOOM);

    const previewBox = vpreview.current.getBoundingClientRect();
    ...
    const fetchCamera = async () => {
      ...
      try {
        ...
        setError(true);
        setErrorMessage("An error occurred accessing the device camera");
      }
    };
  ...
  }, [videoStream, resetSnapCounter, setupForCapturing]);
      ...
      {/* If scanning should be occurring */}
      {
        <div>
          ...
          {sessionYieldedMatch !== false && (
            <canvas id="video-preview" ref={vpreview} />
          )}
...
//         this.cameraCapabilities = track.getCapabilities && track.getCapabilities();
